...
const RUNNING_STATE_FILE = path.join(DATA_DIR, 'running_state.json');
...
class AutoPoster {
  constructor() {
    ...
    this.scheduler = null; // Will be set when needed
  }
  ...
  async loadQueue() {
    try {
      ...
      if (await fs.pathExists(QUEUE_FILE)) {
        this.postQueue = await fs.readJson(QUEUE_FILE);
  ...
  async saveQueue() {
    try {
      await fs.ensureDir(DATA_DIR);
      await fs.writeJson(QUEUE_FILE, this.postQueue, { spaces: 2 });
    } catch (error) {
  ...
  async loadHistory() {
    try {
      ...
      if (await fs.pathExists(HISTORY_FILE)) {
        this.postedHistory = await fs.readJson(HISTORY_FILE);
  ...
  async saveHistory() {
    try {
      await fs.ensureDir(DATA_DIR);
      await fs.writeJson(HISTORY_FILE, this.postedHistory, { spaces: 2 });
    } catch (error) {
  ...
  async saveRunningState() {
    try {
      await fs.ensureDir(DATA_DIR);
      await fs.writeJson(RUNNING_STATE_FILE, { 
  ...
  async loadRunningState() {
    try {
      if (await fs.pathExists(RUNNING_STATE_FILE)) {
        const state = await fs.readJson(RUNNING_STATE_FILE);
        this.running = state.running || false;
        console.log(`📋 Loaded running state: ${this.running ? '🟢 Running' : '⏸️ Paused'}`);
        return this.running;
      }
    } catch (error) {
...
calculateNextPostTime() {
  ...
  // Get current time and add 1 minute to start from the next minute
  ...
  async fetchNewPosts() {
    // REMOVE the running check - fetch should work regardless
    // if (!this.running) {
    //   console.log(`⏸️ Scheduler stopped, skipping fetch`);
    //   return 0;
    // }
...
async manualPost(content, imageUrl = null) {
  // Completely independent manual post - no scheduler state interaction
  try {
    console.log(`\n  📍 Manual post is posting now!`);
    console.log(`  📝 Content: ${(content || 'Image post').slice(0, 50)}...`);
    ...
    const manualPost = {
      ...
      scheduled_time_kenya: 'Manual - Posting Now',
      source_name: 'Manual Post',
      manual: true
    };

    // Create a job without callback
    const job = new PostJob(manualPost, null);
    const result = await job.postNow();

    if (result && result.success) {
      // Directly read/write to history file WITHOUT using scheduler methods
      try {
        const historyFile = HISTORY_FILE; // From outer scope
        let history = [];
        ...
        console.error('Error saving manual post to history:', fileError);
      }
  ...
  async removePostFromQueue(postId) {
    ...
    const removed = this.postQueue.splice(index, 1)[0];
    await this.saveQueue();
  ...
  async initialize() {
    ...
    // Only set initial state if not already running
    // DON'T force reset to false
  ...
  async start() {
    ...
    if (this.running) {
      console.log(`⚠️ Already running after initialization`);
      return;
    }
    ...
    console.log(`🔴 Running state after start: ${this.running}`);
